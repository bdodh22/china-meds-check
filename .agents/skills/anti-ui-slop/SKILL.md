...
5. **Multi-Language Anti-Slop (多语言防自嗨设计规范)**:
   - Never ship generic mock English into non-English routes (e.g. `/vi/`, `/ja/`, `/ko/`, `/ru/`).
   - Obey the Universal i18n Protocol: **100% Target Language Purity**. A page in Vietnamese must not contain English sentences in data tables, airport walkthroughs, or filter dropdowns.
   - Never use fallback stitching (`item.titleVi || item.titleEn`). If a data entity lacks target language data, do not display half-baked English cards.
   - Always protect badges and pills with `shrink-0 whitespace-nowrap` + `gap-2` to prevent layout collapse in languages with long words.
